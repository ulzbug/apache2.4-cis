...
does_file_exist "$APACHE2BIN"
if [ "$FNRET" != 0 ]; then
	echo "apache2 bin not found at $APACHE2BIN"
	echo "Exiting now"
	exit 100
fi

does_file_exist "$APACHE2CTLBIN"
if [ "$FNRET" != 0 ]; then
	echo "apache2ctl bin not found at $APACHE2CTLBIN"
	echo "Exiting now"
	exit 100
fi

get_apache2_version
if [ "$APACHE2_MAJ_VER" != 2 ]; then
	echo "Your apache version is not supported. Only apache2 is supported"
	echo "Exiting now"
	exit 100
fi
if [ "$APACHE2_MIN_VER" != 4 ]; then
	echo "Your apache2 version is not supported. Only apache2.4 is supported"
	echo "Exiting now"
	exit 100
fi
