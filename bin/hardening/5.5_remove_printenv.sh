...
audit() {
	...
	for DOCUMENT_ROOT in $ALL_DOCUMENTS_ROOT; do
		RESULTS=$(find "$DOCUMENT_ROOT" -type f -name printenv)
		if [ -n "$RESULTS" ]; then
			crit "File printenv has been found in DocumentRoot $DOCUMENT_ROOT"
			OK=0
		fi
	done

	if [ "$OK" = 1 ]; then
		ok "No printenv files found in all DocumentRoot directories"
	fi
}
