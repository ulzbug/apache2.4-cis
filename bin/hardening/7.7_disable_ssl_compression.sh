...
audit() {
	...
	SSLCOMPRESSION_FOUND=$(echo "$APACHE2_CONF" | grep -i "SSLCompression" | wc -l)
	SSLCOMPRESSION_OFF_FOUND=$(echo "$APACHE2_CONF" | grep -i "SSLCompression\soff" | wc -l)

	if [ "$SSLCOMPRESSION_FOUND" = 0 ]; then
		ok "No directive SSLCompression found"
	elif [ "$SSLCOMPRESSION_FOUND" = 1 ] && [ "$SSLCOMPRESSION_OFF_FOUND" = 0 ]; then
		ok "SSLCompression is enabled"
	else
		crit "SSLCompression is not enabled"
	fi
}
