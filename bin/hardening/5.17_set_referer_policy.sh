...
audit() {
	...
	REFERRER_POLICY_FOUND=$(echo "$APACHE2_CONF" | grep -i 'Header set Referrer-Policy' | wc -l)

	if [ "$REFERRER_POLICY_FOUND" = 1 ]; then
		ok "HTTP header Referrer-Policy is set"
	else
		crit "HTTP header Referrer-Policy is not set"
	fi
}
