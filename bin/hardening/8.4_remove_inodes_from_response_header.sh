...
audit() {
	...
	FILEETAG_FOUND=$(echo "$APACHE2_CONF" | grep "FileETag" | grep "all|inode" | wc -l)

	if [ "$FILEETAG_FOUND" = 0 ]; then
		ok "FileETag is not present or does not contain all or inod"
	else
		crit "FileETag is present and contain either all or inode"
	fi
}
