...
audit() {
	for APACHE_DIRECTORY in $APACHE_DIRECTORIES; do
		APACHE_NON_ROOT_FILES=$(find "$APACHE_DIRECTORY" \! -group root)

		if [ -n "$APACHE_NON_ROOT_FILES" ]; then
			crit "Directory $APACHE_DIRECTORY contains non root group files"
		else
			ok "Directory $APACHE_DIRECTORY : all files are owned by root group"
		fi
	done
}
