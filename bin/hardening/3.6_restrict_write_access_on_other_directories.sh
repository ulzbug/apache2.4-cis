...
audit() {
	for APACHE_DIRECTORY in $APACHE_DIRECTORIES; do
		APACHE_WRITE_ACCESS_FILES=$(find -L "$APACHE_DIRECTORY" \! -type l -perm /o=w -ls)

		if [ -n "$APACHE_WRITE_ACCESS_FILES" ]; then
			crit "Directory $APACHE_DIRECTORY contains files writable by all users"
		else
			ok "Directory $APACHE_DIRECTORY : all files are not writable by all users"
		fi
	done
}
