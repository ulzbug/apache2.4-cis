...
audit() {
	...
	if [ "$FNRET" != 0 ]; then
		...
		REQUESTREADTIMEOUT_FOUND=$(echo "$APACHE2_CONF" | grep -Ei "RequestReadTimeout.*header=([0-9-]+)" | wc -l)

		if [ "$REQUESTREADTIMEOUT_FOUND" = 0 ]; then
			ok "RequestReadTimeout directive not found"
		else
			REQUESTREADTIMEOUT=$(echo "$APACHE2_CONF" | grep -i "RequestReadTimeout" | grep -oEi "header=[0-9-]+" | grep -oEi "[0-9-]+")
