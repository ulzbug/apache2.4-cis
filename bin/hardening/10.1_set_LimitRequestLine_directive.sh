...
audit() {
	...
	LIMITREQUESTLINE_FOUND=$(echo "$APACHE2_CONF" | grep -Ei "LimitRequestLine\s+([0-9]+)" | wc -l)

	if [ "$LIMITREQUESTLINE_FOUND" = 0 ]; then
		crit "LimitRequestLine directive not found"
	else
		LIMITREQUESTLINE=$(echo "$APACHE2_CONF" | grep -Eoi "LimitRequestLine\s+([0-9]+)")

		if [ "$LIMITREQUESTLINE" -le 512 ] && [ "$LIMITREQUESTLINE" -gt 0 ]; then
			ok "LimitRequestLine is present and lesser than 512"
		else
			crit "LimitRequestLine is present but greater than 512"
		fi
	fi
}
