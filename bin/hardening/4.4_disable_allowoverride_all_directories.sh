...
audit() {
	...
	ALLOW_OVERRIDE_OCCURENCES=$(echo "$APACHE2_CONF" | grep "AllowOverride" | sed -E 's/\s*AllowOverride\s+(.*)/\1/')
	OK=1

	for ALLOW_OVERRIDE_VALUE in $ALLOW_OVERRIDE_OCCURENCES; do
		if [ "$ALLOW_OVERRIDE_VALUE" != "None" ]; then
			OK=0
		fi
	done

	if [ "$OK" = 0 ]; then
		crit "AllowOverride is not disabled for all directories"
	else
		ok "AllowOverride is disabled for all directories"
	fi
}
